...
@RestController
@CrossOrigin ("*")
public class JsonChatController {
    private static final Logger logger = LoggerFactory.getLogger(JsonChatController.class);
    ...
    private final String indexName = "metadata_index";

    @PostMapping("/jsonschema/connect")
    public Map<String, Object> connect(@RequestBody Map<String, String> dbParams) {
        ...
        try {
            ...
            String metadata = graphQLService.fetchDatabaseSchema(dbParams.get("path"));

            // Step 6: Prepare documents for chunking
            Map<String, Object> document = new HashMap<>();
            document.put("id", dbParams.get("path"));
    ...
    public Map<String, Object> chat(@RequestBody Map<String, Object> chatParams) {
        ...
        try {
            ...
            // String cachedResponse = semanticCacheService.performHybridSearch(message);
            ...
            String analysisResult = llmService.processAnalysis(message, model);
            if(analysisResult.toLowerCase().contains("naive")){
                // Perform semantic search using Elasticsearch
                String schemaMetadata = searchAndRerankService.searchRelatedMetadata("metadata_index", dbParams.get("database"), message);
                // Combine system role, schema metadata, and user message to create an enhanced prompt
                combinedPrompt = (systemRole != null ? systemRole : "") +
                        "\n\nSchema Metadata:\n" + schemaMetadata +
                        "\n\nUser Query:\n" + message;
            ...
            logger.info("Enhanced Prompt: " + combinedPrompt);
