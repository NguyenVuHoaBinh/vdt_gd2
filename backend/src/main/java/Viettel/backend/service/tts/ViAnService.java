package Viettel.backend.service.tts;
...
@Service
public class ViAnService {
    @Value("${vian.api.url}")
    private String VIAN_URL;

    @Value("${vian.api.key}")
    private String VIAN_KEY;

    private final RestTemplate restTemplate;
    private final ObjectMapper objectMapper;

    public ViAnService(RestTemplate restTemplate, ObjectMapper objectMapper) {
        this.restTemplate = restTemplate;
        this.objectMapper = objectMapper;
    }

    public String encodeAudioToBase64(String audio) {
        try {
            // Set up the HTTP headers
            HttpHeaders headers = new HttpHeaders();
            headers.setContentType(MediaType.APPLICATION_JSON);
            headers.setBasicAuth(VIAN_KEY);

            // Prepare the payload for the MLflow model
            Map<String, Object> payload = new HashMap<>();
            payload.put("scripts", Collections.singletonList(audio));
